...
- Installer builds completed on all four architectures. Reviewing the Windows logs uncovered a falsely successful Electron exit after a PDF path-normalization error. The path check now uses canonical local file identity, and the smoke runner requires an explicit completion record as well as a zero exit code. See the [branch’s latest CI runs](https://github.com/npandeya/bookwriter/actions/workflows/ci.yml?query=branch%3Arelease-readiness) for the corrected source/packaged checks; validate the exact revision before tagging.
