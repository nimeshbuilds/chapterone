...
CI uses native Intel Mac, Apple Silicon, Windows x64 and Windows ARM64 runners, plus Linux unit tests. `electron-builder.config.js` derives packaging from `package.json`; release tags require macOS signing configuration or an explicit unsigned-release opt-in. `.github/workflows/release.yml` waits for every platform, generates accurate signing notes and checksums, verifies uploaded file hashes, and publishes the complete release. See [RELEASING.md](RELEASING.md).
