...
CI uses native Intel Mac, Apple Silicon, Windows x64 and Windows ARM64 runners, plus Linux unit tests. `electron-builder.config.js` derives packaging from `package.json`; release tags enforce macOS signing configuration. `.github/workflows/release.yml` waits for every platform before assembling a draft and checksum file. See [RELEASING.md](RELEASING.md).
